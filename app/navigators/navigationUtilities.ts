...
import type { MainTabParamList, NavigationProps } from "./navigationTypes"
...
export function getActiveRouteName(state: NavigationState | PartialState<NavigationState>): string {
  ...
  if (!route.state) return route.name as keyof MainTabParamList

  // Recursive call to deal with nested routers
  return getActiveRouteName(route.state as NavigationState<MainTabParamList>)
}
...
export function useNavigationPersistence(storage: Storage, persistenceKey: string) {
  ...
  const routeNameRef = useRef<keyof MainTabParamList | undefined>(undefined)
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      routeNameRef.current = currentRouteName as keyof MainTabParamList
